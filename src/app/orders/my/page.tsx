...
export default function MyOrdersPage() {
  ...
  // IDs các đơn PENDING đã hết hạn — cập nhật realtime từ countdown
          ...
          <div className="flex items-center gap-4 mb-10 border-b border-white/10 pb-6">
            ...
            <div>
              ...
              <p className="text-gray-400 mt-1">
                Toàn bộ lịch sử đặt vé của bạn
              </p>
            </div>
          </div>
          ...
          {orders.length === 0 ? (
            <div className="text-center py-20 bg-card rounded-3xl border border-white/5">
              ...
              <p className="text-gray-400 mb-6">
                Hãy đặt vé xem phim yêu thích ngay!
              </p>
              <Link
                href="/movies"
            ...
            <div className="space-y-5">
              {orders.map((order, index) => {
                ...
                const dateObj = new Date(
                  order.showtime?.start_time || order.created_at,
                );
                const movie = order.showtime?.movie;
                const room = order.showtime?.room;
                // ✅ FIX: Dùng showtime.id thay vì cinema.name
                const showtimeId = order.showtime?.id;
                const isPending = order.status === "PENDING";
                // isExpired reactive: check cả state client-side lẫn server-side
                const isExpired =
                  isPending &&
                    ...
                    <div className="p-5 flex-1 flex flex-col justify-between">
                      ...
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/5">
                        ...
                        <div className="flex flex-col items-end gap-2">
                          {/* Countdown cho đơn PENDING chưa hết hạn — khi về 0 sẽ ẩn nút ngay */}
                          ...
                          {/* ✅ FIX: Link đúng với showtime_id + order_id */}
                          {isPending && !isExpired && showtimeId && (
                            <Link
                              href={`/booking/checkout/${showtimeId}?order_id=${order.id}`}
                            >
