    ...
    const fetchData = async () => {
      try {
        const [personRes, homeMoviesRes] = await Promise.all([
          apiClient.get<void, { success: boolean; data: Person }>(`/persons/${id}`),
    ...
    return (
      <div className="min-h-screen bg-black flex flex-col items-center">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-white space-y-4">
          <h2 className="text-2xl font-bold">Không tìm thấy thông tin diễn viên.</h2>
      ...
      <main className="max-w-7xl mx-auto px-4 lg:px-8 py-12 lg:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            >
              <Image
                src={person.profile_path || "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=500&auto=format&fit=crop"}
                alt={person.name}
                fill
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{person.known_for || "Acting"}</p>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-12">
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <div className="flex items-center gap-2 mb-8">
                ...
                <span className="text-primary font-bold uppercase tracking-widest text-sm">Biography</span>
              </div>

              <div className="prose prose-invert max-w-none">
                <p className="text-gray-400 text-lg leading-relaxed whitespace-pre-line">
                  {person.biography || "Thông tin tiểu sử hiện đang được cập nhật..."}
                </p>
              </div>
            </motion.div>

            {suggestedMovies.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="pt-12 border-t border-white/10">
                <div className="flex items-center justify-between mb-8">
                  <h2 className="text-2xl font-black uppercase tracking-tighter">Phim có thể bạn thích</h2>
                  <Link href="/movies" className="text-xs font-bold uppercase tracking-widest text-primary hover:opacity-80 transition-opacity">
                    Xem tất cả →
                  </Link>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  {suggestedMovies.map((movie) => (
                    <motion.div key={movie.id} whileHover={{ y: -10 }} className="group">
                      <Link href={`/movies/${movie.id}`}>
                        <div className="relative aspect-[2/3] rounded-2xl overflow-hidden border border-white/10 mb-4">
                          <Image
                            src={movie.poster_url || "https://images.unsplash.com/photo-1485846234645-a62644ef7467?q=80&w=500&auto=format&fit=crop"}
                            alt={movie.title}
                            fill
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 group-hover:opacity-80 transition-opacity flex items-end p-4">
                            <span className="text-xs font-bold bg-primary px-2 py-1 rounded">Đặt vé</span>
                          </div>
                        </div>
                        <h4 className="font-bold text-sm line-clamp-1 group-hover:text-primary transition-colors">{movie.title}</h4>
